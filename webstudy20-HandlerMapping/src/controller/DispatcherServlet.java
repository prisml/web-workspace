...
public class DispatcherServlet extends HttpServlet {
	 ...
	 * Controller Interface type으로 반환 받는다 3. 개별 컨트롤러 구현체의 컨트롤러 메서드(execute)를
	 * 실행한다 4. 예외처리 try catch를 작성하고 예외발생시 서버상에서 메세지 출력하고 error.jsp로
	 * 응답한다(redirect) 5. 컨트롤러 메서드가 실행 후 반환되는 url에 의거 forward 또는 redirect 방식으로
	 * 이동시킨다.
	 * 
	 * @param request
	 * @param response
